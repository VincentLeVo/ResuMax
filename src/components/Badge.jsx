...
export function Badge({ color = "zinc", size = "small", className, ...props }) {
  return (
    <span
      ...
      className={clsx(
        className,
        "inline-flex items-center gap-x-1.5 rounded-md  text-sm/5 font-medium sm:text-xs/5 forced-colors:outline",
        size === "small" ? "px-1.5 py-0.5 text-sm/5" : "px-3 py-2 text-base/5",
        colors[color]
      )}
    />
  );
